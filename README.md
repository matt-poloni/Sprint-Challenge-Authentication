...
1. [x] What is the purpose of using _sessions_?

* Sessions allow you to persist authentication information so that you don't need to re-authenticate credentials with every protected/restricted request made to the server by the client.

1. [x] What does bcrypt do to help us store passwords in a secure manner.

* It allows us to hash passwords so that we aren't storing them as plain text.

1. [x] What does bcrypt do to slow down attackers?

* It hashes in multiple, accumulative rounds.

1. [x] What are the three parts of the JSON Web Token?

* The **header**, which specifies the hashing algorithm used to create the token and classifies the token as a JWT.

* The **payload**, which specifies the subject of the token and carries any additional data (aka 'claims') you'd like stored there.

* The **signature**, which is a hash of the secret held by the server and the header and payload of the token.
